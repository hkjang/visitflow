...
- 사내 SMTP: 관리자 설정·테스트 발송, 방문자·담당자 이메일 발송 규칙(제목 템플릿, 방문증 링크), 담당자별 메일 알림 개인화(도착·승인·반려·취소·승인 대기), 로컬 계정 메일 비밀번호 재설정
...
```bash
docker load < visitflow-v2.7.0.tar.gz
...
```bash
./scripts/release-image.sh 2.7.0
gzip -t visitflow-v2.7.0.tar.gz
```
