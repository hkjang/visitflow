...
- 사내 SMTP: 사내 메일 표준(`mail.*` 설정, 기본 25번 포트·인증 없음·`auto` 보안)을 따르는 관리자 설정·시험 발송·발송 기록, 배경 큐 발송, 사람이 기다리는 이벤트(승인 요청·지연·확정·반려·취소·도착)만 담당자·승인자에게 알리고 자기 행위·같은 방문의 중복은 묶음, 이벤트별 관리자 스위치와 사용자별 개인화, 방문자 이메일 발송 규칙, 로컬 계정 메일 비밀번호 재설정
...
```bash
docker load < visitflow-v2.8.2.tar.gz
...
```bash
./scripts/release-image.sh 2.8.2
gzip -t visitflow-v2.8.2.tar.gz
```
