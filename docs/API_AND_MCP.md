...
- `POST /api/v1/visits/import/preview` CSV/XLSX 방문자 가져오기 검증. 첫 시트의 앞 10행 안에서 `이름`·`휴대전화` 열이 있는 행을 헤더로 삼고, `개인정보동의` 열은 `y/yes/true/1/O/✓/동의/예/네/확인` 같은 긍정 표기를 동의로 읽는다. 같은 필드의 열이 반복되면(`휴대전화`·`연락처` 등) 오른쪽 마지막 열을 사용하며, 그 값이 비어 있어도 앞 열로 대체하지 않는다. 미리보기 경고 맨 앞에 헤더 행 번호와 중복 열 번호·이름, 실제 사용한 마지막 열을 필드별로 한 번 안내한다. 경고의 행 번호는 엑셀에서 보이는 행 번호다.
...
- `GET /api/v1/admin/audit-logs.csv|visits.csv|statistics.csv` UTF-8 BOM CSV 내보내기 — 감사 로그 CSV는 목록과 동일하게 `action`·`actor`·`from`·`to`를 적용하고 `limit`(기본 10,000, 최대 100,000)까지 내려받는다. 방문 이력 CSV는 `days`(기본 90, 최대 3,650)와 함께 방문 목록과 동일한 `q`(방문번호·회사·담당자·방문자 이름/전화) 검색과 `status` 필터를 적용하고 `limit`(기본이자 최대 50,000)까지 내려받는다. 두 내보내기 모두 한도에 걸려 잘린 경우 마지막 행에 `#`로 시작하는 안내를 덧붙이고 감사 기록의 `truncated`에 남기므로, 완전한 파일과 잘린 파일을 구분할 수 있다. 통계 CSV와 화면 추이 그래프의 날짜 축은 `days`(기본 30, 최대 366)만큼을 사업장 시간대 기준 오늘까지로 잡으므로, 대시보드의 "오늘" 집계와 같은 날짜를 가리킨다. 통계 화면의 요약 타일과 부서·사업장·방문 유형·입실 시간대·신청 경로 집계도 같은 구간을 쓰므로 추이 그래프의 막대 합과 일치한다.
