...
로그인 실패는 요청 IP와 계정 각각에 대해 집계한다. 설정한 횟수를 넘기면 잠금 시간 동안 해당 IP와 계정의 로그인을 차단하고 `429`와 `Retry-After`를 반환하며 감사 로그에 남긴다. 잠금 정보는 데이터베이스에 저장되므로 재시작이나 다중 노드에서도 그대로 유지된다.
...
프로필 메뉴에서 대리 담당자와 종료 시각을 지정하면 그 기간 동안 방문 승인 권한과 방문자 도착 알림이 대리 담당자에게 전달된다. 승인 대기가 `승인 지연 에스컬레이션` 시간을 넘기면 `approval_escalated` 이벤트가 방문당 한 번 발생하므로, 이 이벤트에 규칙을 연결해 보안 담당자나 관리자에게 알릴 수 있다.
