...
const fields: Record<string, Field[]> = {
  ...
  security: [
    { key: "security.session_hours", label: "세션 유효시간 (시간)", type: "number" }, { key: "security.api_key_days", label: "개인 키 기본 유효기간 (일)", type: "number" }, { key: "security.rotation_grace_hours", label: "키 회전 유예시간 (시간)", type: "number" },
  ],
};
...
export function SettingsPage() {
  ...
  return <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 1150, mx: "auto" }}><PageHeader eyebrow="SYSTEM CONTROL" title="시스템 설정" description="배포 환경변수는 PostgreSQL DSN과 Bootstrap 계정 2개뿐이며, 모든 운영 설정은 여기에서 관리합니다." actions={dirty ? <Chip icon={<WarningAmberRounded />} color="warning" label="저장하지 않은 변경" /> : <Chip icon={<CheckCircleRounded />} color="success" variant="outlined" label="모든 변경 저장됨" />} />{message && <Alert severity="success" onClose={() => setMessage("")} sx={{ mb: 2 }}>{message}</Alert>}{error && <Alert severity="error" onClose={() => setError("")} sx={{ mb: 2 }}>{error}</Alert>}
