...
export function VisitFormPage({ walkIn = false }: { walkIn?: boolean }) {
  ...
  const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [created, setCreated] = useState<{ requestNo: string; status: string; passUrls?: string[] } | null>(null);
  ...
  useEffect(() => {
    const saved = localStorage.getItem("visitflow_template");
    if (!saved || walkIn) return;
    try {
      const template = JSON.parse(saved) as { purpose?: string; placeDetail?: string; company?: string };
      setPurpose(template.purpose ?? ""); setPlaceDetail(template.placeDetail ?? "");
      if (template.company) updateVisitor(0, { company: template.company });
    } finally { localStorage.removeItem("visitflow_template"); }
  }, [walkIn]);
  ...
  const submit = async () => {
    ...
    try {
      const body = { siteId, lobbyId, departmentId, hostUserId: walkIn ? hostUserId : undefined, startAt: new Date(startAt).toISOString(), endAt: new Date(endAt).toISOString(), purpose, placeDetail, notes, visitors: visitors.map((v) => ({ ...v, equipment: v.equipment.split(",").map((x) => x.trim()).filter(Boolean) })) };
      const result = await postJSON<{ requestNo: string; status: string; passUrls?: string[] }>(walkIn ? "/api/v1/lobby/walk-ins" : "/api/v1/visits", body); setCreated(result);
    } catch (e) { setError(e instanceof Error ? e.message : "방문을 등록하지 못했습니다"); } finally { setBusy(false); }
  };
  if (created) return <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 900, mx: "auto" }}><Card><CardContent sx={{ p: { xs: 3, md: 6 }, textAlign: "center" }}><Box sx={{ width: 72, height: 72, borderRadius: "50%", bgcolor: "#E4F3EC", color: "success.main", display: "grid", placeItems: "center", mx: "auto", fontSize: 34 }}>✓</Box><Typography variant="h4" sx={{ mt: 2 }}>방문 등록 완료</Typography><Typography color="text.secondary" sx={{ mt: 1 }}>방문번호 <strong>{created.requestNo}</strong> · 상태 {created.status}</Typography>{created.passUrls?.map((url) => <Paper key={url} variant="outlined" sx={{ mt: 2, p: 1.5, display: "flex", alignItems: "center", gap: 1, wordBreak: "break-all" }}><Typography variant="body2" sx={{ flex: 1 }}>{url}</Typography><IconButton onClick={() => void navigator.clipboard.writeText(url)}><ContentCopyRounded /></IconButton></Paper>)}<Alert severity="info" sx={{ mt: 3, textAlign: "left" }}>{created.status === "PENDING_APPROVAL" ? "승인 완료 후 방문자별 QR 방문증이 발급되고 알림 큐에 등록됩니다." : "방문자별 모바일 방문증이 발급되었고 설정된 SMS Adapter로 발송됩니다."}</Alert><Stack direction="row" justifyContent="center" spacing={1} mt={3}><Button onClick={() => navigate(walkIn ? "/lobby" : "/visits")}>목록으로</Button><Button variant="contained" onClick={() => window.location.reload()}>새로 등록</Button></Stack></CardContent></Card></Box>;
  return <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 1100, mx: "auto" }}><PageHeader eyebrow={walkIn ? "WALK-IN" : "NEW VISIT"} title={walkIn ? "현장 방문 등록" : "방문 신청"} description={walkIn ? "예약 없이 도착한 방문자를 담당자와 연결해 등록합니다." : "여러 방문자도 한 번에 등록하고 각각 고유한 QR 방문증을 발급합니다."} />{error && <Alert severity="error" onClose={() => setError("")} sx={{ mb: 2 }}>{error}</Alert>}
    <Card><CardContent sx={{ p: { xs: 2, md: 4 } }}><Typography variant="h6">방문 일정</Typography><Typography variant="body2" color="text.secondary" mb={3}>언제, 어디에서, 누구를 만나는지 입력하세요.</Typography><Grid container spacing={2}>
