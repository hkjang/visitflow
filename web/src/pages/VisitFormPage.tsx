...
type VisitorDraft = { name: string; phone: string; email: string; company: string; title: string; vehicle: string; equipment: string; locale: string; consent: boolean };
const blankVisitor = (): VisitorDraft => ({ name: "", phone: "", email: "", company: "", title: "", vehicle: "", equipment: "", locale: "", consent: true });
...
export function VisitFormPage({ walkIn = false }: { walkIn?: boolean }) {
  ...
  return <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 1100, mx: "auto" }}><PageHeader eyebrow={walkIn ? "WALK-IN" : "NEW VISIT"} title={walkIn ? "현장 방문 등록" : "방문 신청"} description={walkIn ? "예약 없이 도착한 방문자를 담당자와 연결해 등록합니다." : "여러 방문자도 한 번에 등록하고 각각 고유한 QR 방문증을 발급합니다."} />{error && <Alert severity="error" onClose={() => setError("")} sx={{ mb: 2 }}>{error}</Alert>}{templateLoading && <Alert severity="info" sx={{ mb: 2 }}>선택한 템플릿과 자주 방문자를 불러오는 중입니다.</Alert>}
    <Card sx={{ opacity: templateLoading ? 0.65 : 1, pointerEvents: templateLoading ? "none" : "auto" }}><CardContent sx={{ p: { xs: 2, md: 4 } }}><Typography variant="h6">방문 일정</Typography><Typography variant="body2" color="text.secondary" mb={3}>언제, 어디에서, 누구를 만나는지 입력하세요.</Typography><Grid container spacing={2}>
      ...
      <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth required type="datetime-local" label="방문 시작" value={startAt} onChange={(e) => setStartAt(e.target.value)} error={scheduleMessage !== ""} helperText={scheduleMessage || undefined} slotProps={{ inputLabel: { shrink: true } }} /></Grid><Grid size={{ xs: 12, md: 6 }}><TextField fullWidth required type="datetime-local" label="방문 종료" value={endAt} onChange={(e) => setEndAt(e.target.value)} error={scheduleMessage !== ""} helperText={scheduleMessage || undefined} slotProps={{ inputLabel: { shrink: true } }} /></Grid>
    ...
    <Stack spacing={2}>{visitors.map((visitor, index) => <Paper key={index} variant="outlined" sx={{ p: { xs: 2, md: 3 }, borderRadius: 3 }}><Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}><Stack direction="row" spacing={1} alignItems="center"><Typography fontWeight={800}>방문자 {index + 1}</Typography>{index === 0 && <Chip size="small" label="대표 방문자" color="primary" variant="outlined" />}</Stack><IconButton color="error" disabled={visitors.length === 1} onClick={() => setVisitors((x) => x.filter((_, i) => i !== index))}><DeleteOutlineRounded /></IconButton></Stack><Grid container spacing={2}>
      <Grid size={{ xs: 12, md: 4 }}><TextField fullWidth required label="이름" value={visitor.name} onChange={(e) => updateVisitor(index, { name: e.target.value })} /></Grid><Grid size={{ xs: 12, md: 4 }}><TextField fullWidth required label="휴대전화" value={visitor.phone} onChange={(e) => updateVisitor(index, { phone: e.target.value })} placeholder="010-0000-0000" /></Grid><Grid size={{ xs: 12, md: 4 }}><TextField fullWidth required={companyRequired} helperText={companyRequired ? "현재 정책상 회사명은 필수입니다" : undefined} label="회사명" value={visitor.company} onChange={(e) => updateVisitor(index, { company: e.target.value })} /></Grid>
    ...
    </Grid></Paper>)}</Stack><Divider sx={{ my: 3 }} /><Stack direction={{ xs: "column-reverse", sm: "row" }} justifyContent="flex-end" spacing={1}><Button onClick={() => navigate(-1)}>취소</Button><Button variant="contained" endIcon={<SendRounded />} disabled={busy || templateLoading || !siteId || !purpose || !checklistSatisfied || !declarationsSatisfied || !companiesSatisfied || scheduleMessage !== "" || visitors.some((x) => !x.name || !x.phone || !x.consent) || (walkIn && !hostUserId)} onClick={() => void submit()}>{busy ? "등록 중…" : walkIn ? "현장 방문 등록" : "방문 신청 제출"}</Button></Stack></CardContent></Card>
  </Box>;
}
