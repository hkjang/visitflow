...
function VisitAdmin({ setError }: { setError: (v: string) => void }) {
  const [items, setItems] = useState<Visit[]>([]); const [nextCursor, setNextCursor] = useState(""); const [visitors, setVisitors] = useState<Record<string, unknown>[]>([]); const [tab, setTab] = useState<"visits" | "visitors" | "watchlist">("visits"); const [watch, setWatch] = useState<Record<string, unknown>[]>([]); const [watchOpen, setWatchOpen] = useState(false); const [watchForm, setWatchForm] = useState({ name: "", phone: "", company: "", reason: "" });
  const [query, setQuery] = useState(""); const [visitorQuery, setVisitorQuery] = useState("");
  const load = useCallback(async () => {
    try {
      const visitParams = new URLSearchParams({ limit: "100" }); if (query) visitParams.set("q", query);
      const visitorParams = new URLSearchParams(); if (visitorQuery) visitorParams.set("q", visitorQuery);
      const [v, people, wl] = await Promise.all([api<{ items: Visit[]; nextCursor: string }>(`/api/v1/visits?${visitParams}`), api<{ items: Record<string, unknown>[] }>(`/api/v1/admin/visitors?${visitorParams}`), api<{ items: Record<string, unknown>[] }>("/api/v1/admin/watchlist")]);
      setItems(v.items); setNextCursor(v.nextCursor ?? ""); setVisitors(people.items); setWatch(wl.items);
    } catch (e) { setError(e instanceof Error ? e.message : "데이터를 불러오지 못했습니다"); }
  }, [query, visitorQuery, setError]);
  useEffect(() => { void load(); }, [load]);
  const loadMore = async () => { if (!nextCursor) return; try { const params = new URLSearchParams({ limit: "100", cursor: nextCursor }); if (query) params.set("q", query); const v = await api<{ items: Visit[]; nextCursor: string }>(`/api/v1/visits?${params}`); setItems((x) => [...x, ...v.items]); setNextCursor(v.nextCursor ?? ""); } catch (e) { setError(e instanceof Error ? e.message : "다음 목록을 불러오지 못했습니다"); } };
  ...
  const releaseWatch = async (id: string) => { if (!window.confirm("이 제한을 해제할까요? 해제 이력은 감사 로그에 남습니다.")) return; try { await api(`/api/v1/admin/watchlist/${id}`, { method: "DELETE" }); await load(); } catch (e) { setError(e instanceof Error ? e.message : "해제하지 못했습니다"); } };
  return <><PageHeader eyebrow="VISITOR CONTROL" title="방문 · 방문자 관리" description="전체 방문 Workflow, 방문자 이력과 보안 Watch List를 관리합니다." actions={<><Button startIcon={<DownloadRounded />} href="/api/v1/admin/visits.csv?days=90">방문 이력 CSV</Button>{tab === "watchlist" && <Button variant="contained" startIcon={<AddRounded />} onClick={() => setWatchOpen(true)}>제한 등록</Button>}</>} /><Stack direction={{ xs: "column", md: "row" }} spacing={1} mb={2} alignItems={{ md: "center" }}><Stack direction="row" spacing={1}>{[["visits", "전체 방문"], ["visitors", "방문자 이력"], ["watchlist", "Watch List"]].map(([value, label]) => <Button key={value} variant={tab === value ? "contained" : "outlined"} onClick={() => setTab(value as typeof tab)}>{label}</Button>)}</Stack>{tab === "visits" && <TextField size="small" placeholder="방문번호 / 회사 / 담당자 / 방문자 이름·전화 검색 후 Enter" value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void load(); }} sx={{ flex: 1, minWidth: 260 }} />}{tab === "visitors" && <TextField size="small" placeholder="이름 / 전화번호(정확히) 또는 회사 검색 후 Enter" value={visitorQuery} onChange={(e) => setVisitorQuery(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void load(); }} sx={{ flex: 1, minWidth: 260 }} />}</Stack><TableContainer component={Paper} variant="outlined"><Table size="small"><TableHead><TableRow>{tab === "visits" ? <><TableCell>일시 / 번호</TableCell><TableCell>방문자</TableCell><TableCell>담당 / 장소</TableCell><TableCell>상태</TableCell><TableCell align="right">승인</TableCell></> : tab === "visitors" ? <><TableCell>방문자</TableCell><TableCell>전화번호</TableCell><TableCell>회사</TableCell><TableCell>방문 횟수</TableCell><TableCell>최근 방문</TableCell></> : <><TableCell>대상</TableCell><TableCell>회사</TableCell><TableCell>제한 사유</TableCell><TableCell>기간</TableCell><TableCell>상태</TableCell><TableCell align="right">관리</TableCell></>}</TableRow></TableHead><TableBody>
      {tab === "visits" && items.map((x) => <TableRow key={x.id}><TableCell><Typography variant="body2" fontWeight={700}>{new Date(x.startAt).toLocaleString("ko-KR")}</Typography><Typography variant="caption" sx={{ fontFamily: "monospace" }}>{x.requestNo}</Typography></TableCell><TableCell>{x.primaryVisitor}{x.visitorCount > 1 ? ` 외 ${x.visitorCount - 1}명` : ""}<Typography variant="caption" display="block" color="text.secondary">{x.company}</Typography></TableCell><TableCell>{x.hostName}<Typography variant="caption" display="block" color="text.secondary">{x.siteName} · {x.lobbyName}{x.visitTypeName ? ` · ${x.visitTypeName}` : ""}</Typography></TableCell><TableCell><StatusChip status={x.status} /></TableCell><TableCell align="right">{x.status === "PENDING_APPROVAL" && <Stack direction="row" justifyContent="flex-end"><Button size="small" color="error" onClick={() => void approval(x, false)}>반려</Button><Button size="small" onClick={() => void approval(x, true)}>승인</Button></Stack>}</TableCell></TableRow>)}
...
function Resources({ setError }: { setError: (v: string) => void }) {
  ...
  const editSite = (x: Record<string, unknown>) => { setForm({ id: String(x.id), code: String(x.code ?? ""), name: String(x.name ?? ""), address: String(x.address ?? ""), mapUrl: String(x.mapUrl ?? ""), timezone: String(x.timezone ?? "Asia/Seoul") }); setOpen("site"); };
  const editLobby = (x: Record<string, unknown>) => { setForm({ id: String(x.id), siteId: String(x.siteId ?? ""), code: String(x.code ?? ""), name: String(x.name ?? ""), instructions: String(x.instructions ?? "") }); setOpen("lobby"); };
  const editDepartment = (x: Record<string, unknown>) => { setForm({ id: String(x.id), name: String(x.name ?? ""), parentId: String(x.parentId ?? ""), color: String(x.color ?? "") }); setOpen("department"); };
  const roles = ["user", "lobby", "dept_manager", "security", "auditor", "admin", "super_admin"].filter((role) => role !== "super_admin" || me?.role === "super_admin");
  return <><PageHeader eyebrow="MASTER DATA" title="조직 · 사업장 · 권한" description="멀티 사업장과 로비, 조직 및 Keycloak 프로비저닝 사용자를 관리합니다." /><Grid container spacing={2}><Grid size={{ xs: 12, lg: 4 }}><ResourceCard title="사업장" items={(ref?.sites ?? []) as unknown as Record<string, unknown>[]} label={(x) => `${String(x.name)} · ${String(x.timezone ?? "")}`} onAdd={() => { setForm({ timezone: "Asia/Seoul" }); setOpen("site"); }} onEdit={editSite} /></Grid><Grid size={{ xs: 12, lg: 4 }}><ResourceCard title="로비" items={(ref?.lobbies ?? []) as unknown as Record<string, unknown>[]} label={(x) => `${String(x.name)} · ${ref?.sites.find((site) => site.id === x.siteId)?.name ?? ""}`} onAdd={() => { setForm({ siteId: ref?.sites[0]?.id ?? "" }); setOpen("lobby"); }} onEdit={editLobby} /></Grid><Grid size={{ xs: 12, lg: 4 }}><ResourceCard title="조직" items={(ref?.departments ?? []) as unknown as Record<string, unknown>[]} label={(x) => String(x.name)} onAdd={() => { setForm({}); setOpen("department"); }} onEdit={editDepartment} /></Grid></Grid><Card sx={{ mt: 3 }}><CardContent><Typography variant="h6">사용자 · RBAC</Typography><Typography variant="body2" color="text.secondary" mb={2}>Role, 소속 부서(부서 관리자 승인 범위), 담당 사업장(로비 담당자 범위)과 계정 활성 상태를 변경합니다. 최고 관리자 계정은 최고 관리자만 바꿀 수 있습니다.</Typography><TableContainer component={Paper} variant="outlined"><Table size="small"><TableHead><TableRow><TableCell>사용자</TableCell><TableCell>아이디</TableCell><TableCell>인증</TableCell><TableCell>Role</TableCell><TableCell>부서</TableCell><TableCell>담당 사업장</TableCell><TableCell>상태</TableCell><TableCell>최근 로그인</TableCell></TableRow></TableHead><TableBody>{users.map((u) => { const locked = u.role === "super_admin" && me?.role !== "super_admin"; const scope = Array.isArray(u.siteScope) ? (u.siteScope as string[]) : []; return <TableRow key={String(u.id)} sx={{ opacity: u.active === false ? .55 : 1 }}><TableCell>{String(u.displayName)}</TableCell><TableCell>{String(u.username)}</TableCell><TableCell>{String(u.source).toUpperCase()}</TableCell><TableCell><Stack spacing={.5} alignItems="flex-start"><TextField select disabled={locked} value={String(u.role)} onChange={(e) => void updateUser(String(u.id), { role: e.target.value })} sx={{ minWidth: 150 }}>{(locked ? ["super_admin"] : roles).map((role) => <MenuItem key={role} value={role}>{role}</MenuItem>)}</TextField>{u.source === "oidc" && u.roleOverride === true && <Button size="small" onClick={() => void updateUser(String(u.id), { useOidcMapping: true })}>SSO 그룹 매핑으로 복귀</Button>}</Stack></TableCell><TableCell><TextField select disabled={locked} value={String(u.departmentId ?? "")} onChange={(e) => void updateUser(String(u.id), { departmentId: e.target.value })} sx={{ minWidth: 140 }}><MenuItem value="">미지정</MenuItem>{ref?.departments.map((d) => <MenuItem key={d.id} value={d.id}>{d.name}</MenuItem>)}</TextField></TableCell><TableCell><TextField select disabled={locked} value={scope} onChange={(e) => void updateUser(String(u.id), { siteScope: typeof e.target.value === "string" ? e.target.value.split(",").filter(Boolean) : e.target.value })} slotProps={{ select: { multiple: true, displayEmpty: true, renderValue: (selected) => { const chosen = selected as string[]; return chosen.length === 0 ? "전체" : chosen.map((id) => ref?.sites.find((x) => x.id === id)?.name ?? id).join(", "); } } }} sx={{ minWidth: 150 }}>{ref?.sites.map((x) => <MenuItem key={x.id} value={x.id}>{x.name}</MenuItem>)}</TextField></TableCell><TableCell><Button size="small" disabled={locked || u.id === me?.id} color={u.active === false ? "success" : "error"} variant="outlined" onClick={() => void updateUser(String(u.id), { active: u.active === false })}>{u.active === false ? "활성화" : "비활성화"}</Button></TableCell><TableCell>{u.lastLoginAt ? new Date(String(u.lastLoginAt)).toLocaleString("ko-KR") : "-"}</TableCell></TableRow>; })}</TableBody></Table></TableContainer></CardContent></Card>
    <Dialog open={Boolean(open)} onClose={() => setOpen(null)} fullWidth><DialogTitle>{open === "site" ? "사업장" : open === "lobby" ? "로비" : "조직"} {form.id ? "수정" : "추가"}</DialogTitle><DialogContent><Stack spacing={2} mt={1}>{open === "lobby" && <TextField select label="사업장" value={form.siteId ?? ""} onChange={(e) => setForm({ ...form, siteId: e.target.value })}>{ref?.sites.map((x) => <MenuItem key={x.id} value={x.id}>{x.name}</MenuItem>)}</TextField>}{open !== "department" && <TextField required label="코드" value={form.code ?? ""} onChange={(e) => setForm({ ...form, code: e.target.value })} />}<TextField required label="이름" value={form.name ?? ""} onChange={(e) => setForm({ ...form, name: e.target.value })} />{open === "site" && <><TextField label="주소" value={form.address ?? ""} onChange={(e) => setForm({ ...form, address: e.target.value })} /><TextField label="지도 URL" value={form.mapUrl ?? ""} onChange={(e) => setForm({ ...form, mapUrl: e.target.value })} /><TextField label="시간대 (IANA)" value={form.timezone ?? ""} onChange={(e) => setForm({ ...form, timezone: e.target.value })} helperText="예: Asia/Seoul, Asia/Tokyo. 오늘 방문·통계·CSV의 날짜 기준입니다." /></>}{open === "lobby" && <TextField multiline label="방문 안내" value={form.instructions ?? ""} onChange={(e) => setForm({ ...form, instructions: e.target.value })} />}{open === "department" && <TextField select label="상위 조직" value={form.parentId ?? ""} onChange={(e) => setForm({ ...form, parentId: e.target.value })}><MenuItem value="">없음</MenuItem>{(ref?.departments ?? []).filter((d) => d.id !== form.id).map((d) => <MenuItem key={d.id} value={d.id}>{d.name}</MenuItem>)}</TextField>}</Stack></DialogContent><DialogActions><Button onClick={() => setOpen(null)}>취소</Button><Button variant="contained" disabled={!form.name || (open !== "department" && !form.code)} onClick={() => void save()}>저장</Button></DialogActions></Dialog></>;
}
function ResourceCard({ title, items, label, onAdd, onEdit }: { title: string; items: Record<string, unknown>[]; label: (x: Record<string, unknown>) => string; onAdd: () => void; onEdit: (x: Record<string, unknown>) => void }) { return <Card sx={{ height: "100%" }}><CardContent><Stack direction="row" justifyContent="space-between" alignItems="center"><Typography variant="h6">{title}</Typography><Button size="small" startIcon={<AddRounded />} onClick={onAdd}>추가</Button></Stack><Divider sx={{ my: 2 }} /><Stack spacing={1}>{items.map((x) => <Paper key={String(x.id)} variant="outlined" sx={{ p: 1.5, cursor: "pointer", "&:hover": { bgcolor: "#F3F7F5" } }} onClick={() => onEdit(x)} title="클릭해서 수정"><Typography variant="body2" fontWeight={700}>{label(x)}</Typography></Paper>)}{items.length === 0 && <Typography color="text.secondary">등록 항목 없음</Typography>}</Stack></CardContent></Card>; }
  ...
  const load = useCallback(async () => {
    try {
      const [statistics, list] = await Promise.all([
        ...
        api<{ items: Record<string, unknown>[] }>(`/api/v1/admin/notifications?limit=300${statusFilter ? `&status=${statusFilter}` : ""}`),
      ]);
...
function Audit({ setError }: { setError: (v: string) => void }) {
  const [items, setItems] = useState<Record<string, unknown>[]>([]); const [nextBefore, setNextBefore] = useState(0);
  const [action, setAction] = useState(""); const [actor, setActor] = useState(""); const [from, setFrom] = useState(""); const [to, setTo] = useState("");
  const buildQuery = useCallback((before?: number) => { const params = new URLSearchParams({ limit: "200" }); if (action) params.set("action", action); if (actor) params.set("actor", actor); if (from) params.set("from", new Date(from).toISOString()); if (to) params.set("to", new Date(to).toISOString()); if (before) params.set("before", String(before)); return params; }, [action, actor, from, to]);
  const load = useCallback(async () => { try { const data = await api<{ items: Record<string, unknown>[]; nextBefore: number }>(`/api/v1/admin/audit-logs?${buildQuery()}`); setItems(data.items); setNextBefore(data.nextBefore ?? 0); } catch (e) { setError(e instanceof Error ? e.message : "감사 로그를 불러오지 못했습니다"); } }, [buildQuery, setError]);
  useEffect(() => { void load(); }, [load]);
  const loadMore = async () => { if (!nextBefore) return; try { const data = await api<{ items: Record<string, unknown>[]; nextBefore: number }>(`/api/v1/admin/audit-logs?${buildQuery(nextBefore)}`); setItems((x) => [...x, ...data.items]); setNextBefore(data.nextBefore ?? 0); } catch (e) { setError(e instanceof Error ? e.message : "다음 로그를 불러오지 못했습니다"); } };
  const exportQuery = new URLSearchParams(); if (action) exportQuery.set("action", action);
  return <><PageHeader eyebrow="IMMUTABLE TRAIL" title="Audit Log" description="로그인, 개인정보 조회, QR 검증과 모든 운영 변경을 추적합니다." actions={<Button startIcon={<DownloadRounded />} href={`/api/v1/admin/audit-logs.csv?${exportQuery}`}>감사 로그 CSV</Button>} /><Stack direction={{ xs: "column", md: "row" }} spacing={1} mb={2}><TextField size="small" label="이벤트 접두어" placeholder="visit. / auth. / settings." value={action} onChange={(e) => setAction(e.target.value)} sx={{ minWidth: 200 }} /><TextField size="small" label="행위자" value={actor} onChange={(e) => setActor(e.target.value)} sx={{ minWidth: 160 }} /><TextField size="small" type="datetime-local" label="시작" value={from} onChange={(e) => setFrom(e.target.value)} slotProps={{ inputLabel: { shrink: true } }} /><TextField size="small" type="datetime-local" label="종료" value={to} onChange={(e) => setTo(e.target.value)} slotProps={{ inputLabel: { shrink: true } }} /></Stack><TableContainer component={Paper} variant="outlined"><Table size="small"><TableHead><TableRow><TableCell>시간</TableCell><TableCell>행위자</TableCell><TableCell>이벤트</TableCell><TableCell>대상</TableCell><TableCell>IP</TableCell><TableCell>상세</TableCell></TableRow></TableHead><TableBody>{items.map((x) => <TableRow key={String(x.id)}><TableCell sx={{ whiteSpace: "nowrap" }}>{new Date(String(x.createdAt)).toLocaleString("ko-KR")}</TableCell><TableCell>{String(x.actor)}</TableCell><TableCell><Chip size="small" label={String(x.action)} /></TableCell><TableCell>{String(x.resourceType)} · {String(x.resourceId || "-")}</TableCell><TableCell>{String(x.ipAddress)}</TableCell><TableCell><Typography variant="caption" sx={{ fontFamily: "monospace", wordBreak: "break-all" }}>{JSON.stringify(x.details)}</Typography></TableCell></TableRow>)}{items.length === 0 && <TableRow><TableCell colSpan={6} align="center" sx={{ py: 6, color: "text.secondary" }}>조건에 맞는 기록이 없습니다.</TableCell></TableRow>}</TableBody></Table></TableContainer>{nextBefore > 0 && <Stack direction="row" justifyContent="center" mt={2}><Button onClick={() => void loadMore()}>더 보기</Button></Stack>}</>;
}
