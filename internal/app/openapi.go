...
func (s *Server) openAPI(w http.ResponseWriter, _ *http.Request) {
	paths := map[string]any{
		...
		"/visits/{visitID}/cancel-series":              map[string]any{"post": operation("이 회차부터 반복 일정 일괄 취소", true)},
		"/visitor-visits/{visitorVisitID}/cancel":      map[string]any{"post": operation("단체 방문 중 방문자 개별 취소", true)},
		"/checkins/manual":                             map[string]any{"post": operation("QR 없는 방문자 신분 확인 후 직접 체크인", true)},
		...
		"/settings":                                    map[string]any{"get": operation("전체 운영 설정 조회", true), "put": operation("운영 설정 변경(내보내기 파일 그대로 가져오기 가능)", true)},
		"/settings/export":                             map[string]any{"get": operation("비밀값 제외 설정 JSON 내보내기", true)},
		"/settings/smtp/test":                          map[string]any{"post": operation("SMTP 테스트 메일 발송", true)},
		"/tracking/csp-report":                         map[string]any{"post": operation("브라우저의 콘텐츠 보안 정책 위반 신고 수신", false)},
		"/admin/tracking/violations":                   map[string]any{"get": operation("방문 추적 중 정책이 차단한 출처 목록", true), "delete": operation("차단 출처 기록 비우기", true)},
		"/admin/tracking/allow":                        map[string]any{"post": operation("차단된 출처를 추적 허용 목록에 추가", true)},
		"/auth/password-reset/request":                 map[string]any{"post": operation("로컬 계정 비밀번호 재설정 메일 요청", false)},
		"/auth/password-reset/{token}":                 map[string]any{"get": operation("재설정 링크 유효성", false), "post": operation("재설정 링크로 새 비밀번호 설정", false)},
		"/profile/notifications":                       map[string]any{"get": operation("내 메일 알림 설정 조회", true), "put": operation("내 메일 알림 설정 변경", true)},
		"/admin/users":                                 map[string]any{"get": operation("사용자 목록", true), "post": operation("로컬 사용자 생성(임시 비밀번호 1회 표시)", true)},
		"/admin/users/{userID}/password-reset":         map[string]any{"post": operation("임시 비밀번호 발급 및 전체 세션 종료", true)},
		"/admin/users/{userID}/sessions/revoke":        map[string]any{"post": operation("세션·개인 API 키 즉시 폐기", true)},
		"/admin/visitors/{visitorID}":                  map[string]any{"get": operation("방문자 이력·동의 기록 조회", true)},
		"/admin/visitors/{visitorID}/erase":            map[string]any{"post": operation("삭제 요청에 따른 개인정보 즉시 파기", true)},
		"/admin/notification-apis/{apiID}/test":        map[string]any{"post": operation("문자 API 테스트 발송", true)},
